...
class QuranAudioDownloader:
    ...
    def get_chapter_recitation(self, reciter_id, chapter_id):
        """Get a specific chapter audio file for a reciter."""
        data = self._make_request(f"chapter_recitations/{reciter_id}/{chapter_id}")
        if not data:
            return None
    ...
    def _create_chapter_audio_json(self, reciter_id, reciter_info):
        ...
        # Get all chapter audio files for this reciter
        ...
        # Process each chapter from the bulk endpoint
        ...
        # Now check for individual chapters that might be missing
        # This handles the specific URL format mentioned by the user
        for chapter_number in range(1, 115):  # Quran has 114 chapters
            # Skip if this chapter is already in the list
            if any(chapter["chapter_id"] == chapter_number for chapter in chapter_list["chapters"]):
                continue

            # Try to get the specific chapter audio file
            chapter_file = self.get_chapter_recitation(reciter_id, chapter_number)
            if not chapter_file:
                continue
